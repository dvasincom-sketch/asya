...
export type ChatCfg = {
  ...
  rules: string | null; repoUrl: string | null; enabled: boolean; commands: string | null; caps: string | null; updatedAt?: string;
};
...
function synthetic(id: string, title?: string | null): ChatCfg {
  return { chatId: id, title: title ?? null, role: envIds().includes(id) ? "both" : "support", space: "default", rules: null, repoUrl: null, enabled: true, commands: null, caps: null };
}
...
export async function updateChatConfig(chatId: string, data: Partial<ChatCfg>): Promise<ChatCfg | null> {
  ...
  for (const k of ["title", "role", "space", "rules", "repoUrl", "enabled", "commands", "caps"] as const) {
