...
function supportSystem(kbText: string): string {
...
Правила и дух сообщества (соблюдай их в ответах и мягко направляй к ним): ${COMMUNITY_RULES}
...
export async function communitySupportReply(text: string, space: string): Promise<string> {
  const t = (text || "").trim();
  if (!t) return "";
  const articles = await listArticles(space);
  const raw = await complete([{ role: "user", content: t.slice(0, 1000) }], supportSystem(buildKbContext(articles)), 400).catch(() => "");
  const out = (raw || "").trim();
